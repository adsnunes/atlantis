// atlantis_backend_posix.c - POSIX Backend
...
// SVG support via atlantis_svg
#include "atlantis_svg.h"

// Common terminal definitions
#include "atlantis_term_common.h"
...
static struct {
    ...
    AtlantisMode mode; //!< Interactive or print mode
    ...
    AtlantisColor* print_bg; //!< Default background for print mode margins (NULL if not set)
} posix_state = { 0 };
...
static inline void buf_cursor(int32_t row, int32_t col)
{
    ...
    if (posix_state.mode == ATLANTIS_MODE_PRINT) {
...
{
    if (posix_state.mode == ATLANTIS_MODE_PRINT && posix_state.tty_fd >= 0) {
...
static bool query_background_color(AtlantisColor* out)
{
    if (!out)
        return false;
...
{
    posix_state.capabilities = ATLANTIS_CAP_NONE;

    const char* colorterm = getenv("COLORTERM");
    if (colorterm && (strcmp(colorterm, "truecolor") == 0 || strcmp(colorterm, "24bit") == 0)) {
        posix_state.capabilities |= ATLANTIS_CAP_TRUE_COLOR;
    }

    if (query_mode_supported(2026)) {
        posix_state.capabilities |= ATLANTIS_CAP_SYNC_OUTPUT;
    }

    if (query_mode_supported(2004)) {
        posix_state.capabilities |= ATLANTIS_CAP_BRACKETED_PASTE;
    }

    if (query_kitty_keyboard()) {
        // Implies styled underlines too
        posix_state.capabilities |= ATLANTIS_CAP_STYLED_UNDERLINE;
    }

    if (query_kitty_graphics()) {
        posix_state.capabilities |= ATLANTIS_CAP_IMAGES;
    }

    if (query_text_sizing()) {
        posix_state.capabilities |= ATLANTIS_CAP_TEXT_SIZING;
    }

    // Mouse and clipboard always available on POSIX
    posix_state.capabilities |= ATLANTIS_CAP_MOUSE;
    posix_state.capabilities |= ATLANTIS_CAP_CLIPBOARD;

    drain_input();
}

static bool posix_init(AtlantisMode mode)
{
    ...
    if (mode == ATLANTIS_MODE_PRINT) {
        ...
        AtlantisColor term_bg;
        if (query_background_color(&term_bg)) {
            posix_state.print_bg = malloc(sizeof(AtlantisColor));
            if (posix_state.print_bg) {
    ...
    if (posix_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
...
static AtlantisColor* posix_get_host_bg(void)
{
    if (!posix_state.print_bg)
        return NULL;
    AtlantisColor* copy = malloc(sizeof(AtlantisColor));
    if (copy)
...
static void posix_set_fg(AtlantisColor color)
{
    buf_fg(color.r, color.g, color.b);
}

static void posix_set_bg(AtlantisColor color)
{
    ...
    if (posix_state.mode == ATLANTIS_MODE_PRINT && posix_state.print_bg) {
...
static void posix_set_underline(AtlantisUnderline style)
{
    if (posix_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
        switch (style) {
        case ATLANTIS_UNDERLINE_SINGLE:
            buf_append_str(UNDERLINE);
            break;
        case ATLANTIS_UNDERLINE_CURLY:
            buf_append_str(UNDERLINE_CURLY);
            break;
        case ATLANTIS_UNDERLINE_DOTTED:
            buf_append_str(UNDERLINE_DOTTED);
            break;
        case ATLANTIS_UNDERLINE_DASHED:
...
static void posix_set_underline_color(AtlantisColor color)
{
    if (posix_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
...
static void posix_clear_range(int32_t count)
{
    ...
    if (posix_state.mode == ATLANTIS_MODE_PRINT || count <= 0)
        return;
...
{
    if (scale <= 1 || !(posix_state.capabilities & ATLANTIS_CAP_TEXT_SIZING)) {
        buf_append(str, len);
        if (posix_state.mode == ATLANTIS_MODE_PRINT) {
            posix_state.print_col += utf8_display_width(str, len);
        }
...
{
    if (!(posix_state.capabilities & ATLANTIS_CAP_TEXT_SIZING)) {
        buf_append(str, len);
        if (posix_state.mode == ATLANTIS_MODE_PRINT) {
            posix_state.print_col += utf8_display_width(str, len);
        }
...
{
    if (posix_state.capabilities & ATLANTIS_CAP_SYNC_OUTPUT) {
        ...
        if (seq[0] == '[') {
            ...
            if (seq[1] == '<') {
                ...
                if (sscanf(mouse_buf, "%d;%d;%d", &btn, &mx, &my) == 3) {
                    ...
                    if (btn == 64)
                        return ATLANTIS_KEY_MOUSE_SCROLL_UP;
                    if (btn == 65)
                        return ATLANTIS_KEY_MOUSE_SCROLL_DOWN;
                    // Left button click (btn 0 = press, check for 'M' terminator)
                    if (btn == 0)
                        return ATLANTIS_KEY_MOUSE_CLICK;
                }
                return ATLANTIS_KEY_NONE;
            }
            ...
            if (seq[1] >= '0' && seq[1] <= '9') {
                ...
                if (is_kitty) {
                    ...
                    if (sscanf(peek, "%d;%d%c", &keycode, &mods, &term) >= 2 || sscanf(peek, "%d%c", &keycode, &term) >= 1) {
                        ...
                        case 57352: // Up
                            if (shift)
                                return ATLANTIS_KEY_SHIFT_UP;
                            return ATLANTIS_KEY_UP;
                        case 57353: // Down
                            if (shift)
                                return ATLANTIS_KEY_SHIFT_DOWN;
                            return ATLANTIS_KEY_DOWN;
                        case 57351:
                            if (alt && shift)
                                return ATLANTIS_KEY_ALT_SHIFT_RIGHT;
                            if (alt)
                                return ATLANTIS_KEY_ALT_RIGHT;
                            if (ctrl && shift)
                                return ATLANTIS_KEY_CTRL_SHIFT_RIGHT;
                            if (ctrl)
                                return ATLANTIS_KEY_CTRL_RIGHT;
                            if (shift)
                                return ATLANTIS_KEY_SHIFT_RIGHT;
                            return ATLANTIS_KEY_RIGHT;
                        case 57350:
                            if (alt && shift)
                                return ATLANTIS_KEY_ALT_SHIFT_LEFT;
                            if (alt)
                                return ATLANTIS_KEY_ALT_LEFT;
                            if (ctrl && shift)
                                return ATLANTIS_KEY_CTRL_SHIFT_LEFT;
                            if (ctrl)
                                return ATLANTIS_KEY_CTRL_LEFT;
                            if (shift)
                                return ATLANTIS_KEY_SHIFT_LEFT;
                            return ATLANTIS_KEY_LEFT;
                        case 57360: // Home
                            if (ctrl)
                                return ATLANTIS_KEY_CTRL_HOME;
                            return ATLANTIS_KEY_HOME;
                        case 57367: // End
                            if (ctrl)
                                return ATLANTIS_KEY_CTRL_END;
                            return ATLANTIS_KEY_END;
                        case 57362:
                            return ATLANTIS_KEY_DEL;
                        case 57365:
                            return ATLANTIS_KEY_PGUP;
                        case 57366:
                            return ATLANTIS_KEY_PGDN;
                        case 9:
                            return shift ? ATLANTIS_KEY_BTAB : '\t';
                        case 13:
                            return '\r';
                        case 27:
                            return '\x1b';
...
static bool posix_list_dir(const char* path, char*** out_names, int32_t* out_count)
{
    ...
    *out_count = 0;
    ...
    while ((e = readdir(d))) {
        ...
        names[count++] = atlantis_strdup(e->d_name);
    }
    closedir(d);
...
static int64_t posix_clock(AtlantisClock kind)
{
...
static void posix_get_local_time(AtlantisTime* out)
{
    if (!out)
        return;
...
static void posix_get_local_time_from(AtlantisTime* out, int64_t timestamp)
{
    if (!out)
        return;
...
static uint32_t transmit_to_terminal(const char* path)
{
    ...
    entry->path = atlantis_strdup(path);
...
static void posix_image_mask_region(int32_t col, int32_t row, int32_t cols, int32_t rows, AtlantisColor bg)
{
    if (cols <= 0 || rows <= 0)
        return;
...
static void mark_url_failed(const char* url)
{
    ...
    failed_urls[failed_url_count++] = atlantis_strdup(url);
}
...
static bool start_async_download(const char* url, const char* temp_path, const char* final_path)
{
    ...
    curl_easy_setopt(easy, CURLOPT_USERAGENT, "Atlantis/1.0");

    curl_multi_add_handle(curl_multi_handle, easy);

    AsyncDownload* dl = &downloads[download_count++];
    dl->url = atlantis_strdup(url);
    dl->temp_path = atlantis_strdup(temp_path);
    dl->final_path = atlantis_strdup(final_path);
    dl->fp = fp;
    dl->easy = easy;
...
static bool download_url_to_cache(const char* url, char* cached_path, size_t path_size)
{
    ...
    char cache_dir[512];
    snprintf(cache_dir, sizeof(cache_dir), "%s/.atlantis/image-cache", home);
...
static bool ensure_png_cached(const char* src_path, char* out, size_t out_size)
{
    ...
    // Handle SVG files via atlantis_svg
...
const AtlantisBackend atlantis_backend_posix = {
    .name = "posix",
