// atlantis_backend_win32.c - Windows Backend
...
{
    if (win32_state.mode == ATLANTIS_MODE_PRINT) {
...
{
    if (win32_state.mode == ATLANTIS_MODE_PRINT && win32_state.h_conout != INVALID_HANDLE_VALUE) {
...
{
    if (win32_state.mode == ATLANTIS_MODE_PRINT && win32_state.h_conin != INVALID_HANDLE_VALUE) {
...
static void detect_capabilities(void)
{
    ...
    if (query_mode_supported(2026)) {
        win32_state.capabilities |= ATLANTIS_CAP_SYNC_OUTPUT;
    }

    // Query for bracketed paste
    if (query_mode_supported(2004)) {
        win32_state.capabilities |= ATLANTIS_CAP_BRACKETED_PASTE;
    }

    // Query for Kitty keyboard protocol (implies styled underlines)
    if (query_kitty_keyboard()) {
        win32_state.capabilities |= ATLANTIS_CAP_STYLED_UNDERLINE;
    }

    // Query for Kitty graphics protocol
    if (query_kitty_graphics()) {
        win32_state.capabilities |= ATLANTIS_CAP_IMAGES;
    }

    // Query for text sizing
    if (query_text_sizing()) {
        win32_state.capabilities |= ATLANTIS_CAP_TEXT_SIZING;
    }

    // Mouse and clipboard always available on Windows
    win32_state.capabilities |= ATLANTIS_CAP_MOUSE;
    win32_state.capabilities |= ATLANTIS_CAP_CLIPBOARD;
...
static bool win32_init(AtlantisMode mode)
{
    ...
    if (mode == ATLANTIS_MODE_PRINT) {
        ...
        if (query_background_color(&term_bg)) {
            win32_state.print_bg = malloc(sizeof(AtlantisColor));
            if (win32_state.print_bg) {
    ...
    if (win32_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
...
static AtlantisColor* win32_get_host_bg(void)
{
    if (!win32_state.print_bg)
        return NULL;
...
static void win32_set_fg(AtlantisColor color)
{
    buf_fg(color.r, color.g, color.b);
}

static void win32_set_bg(AtlantisColor color)
{
    if (win32_state.mode == ATLANTIS_MODE_PRINT && win32_state.print_bg) {
...
static void win32_set_underline(AtlantisUnderline style)
{
    if (win32_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
        switch (style) {
...
static void win32_set_underline_color(AtlantisColor color)
{
    if (win32_state.capabilities & ATLANTIS_CAP_STYLED_UNDERLINE) {
...
{
    if (win32_state.mode == ATLANTIS_MODE_PRINT || count <= 0)
        return;
    char buf[16];
...
{
    if (scale <= 1 || !(win32_state.capabilities & ATLANTIS_CAP_TEXT_SIZING)) {
        buf_append(str, len);
        if (win32_state.mode == ATLANTIS_MODE_PRINT) {
            win32_state.print_col += utf8_display_width(str, len);
        }
...
{
    if (!(win32_state.capabilities & ATLANTIS_CAP_TEXT_SIZING)) {
        buf_append(str, len);
        if (win32_state.mode == ATLANTIS_MODE_PRINT) {
            win32_state.print_col += utf8_display_width(str, len);
        }
...
{
    if (win32_state.capabilities & ATLANTIS_CAP_SYNC_OUTPUT) {
    ...
    while (1) {
        ...
        if (rec.EventType == MOUSE_EVENT) {
            ...
            if (mer->dwEventFlags == MOUSE_WHEELED) {
                ...
                return delta > 0 ? ATLANTIS_KEY_MOUSE_SCROLL_UP : ATLANTIS_KEY_MOUSE_SCROLL_DOWN;
            }
        ...
        case VK_HOME:
            return ctrl ? ATLANTIS_KEY_CTRL_HOME : ATLANTIS_KEY_HOME;
        case VK_END:
            return ctrl ? ATLANTIS_KEY_CTRL_END : ATLANTIS_KEY_END;
        case VK_PRIOR:
...
static int64_t win32_clock(AtlantisClock kind)
{
...
static void win32_get_local_time(AtlantisTime* out)
{
    if (!out)
        return;
...
static void win32_get_local_time_from(AtlantisTime* out, int64_t timestamp)
{
    if (!out)
        return;
...
static void win32_image_mask_region(int32_t col, int32_t row, int32_t cols, int32_t rows, AtlantisColor bg)
{
    if (cols <= 0 || rows <= 0)
        return;
...
static bool start_async_download(const char* url, const char* temp_path, const char* final_path)
{
    ...
    HINTERNET h_session = WinHttpOpen(L"Atlantis/1.0",
...
const AtlantisBackend atlantis_backend_win32 = {
    .name = "win32",
