// atlantis_history.c - Session history management
...
static char* normalize_path(const char* path)
{
    if (!path)
        return NULL;
    char* result = atlantis_strdup(path);
#ifdef _WIN32
...
static void format_date(int64_t timestamp, char* buf, size_t len)
{
    ...
    AtlantisTime lt;
    ATLANTIS_BACKEND(app)->localtime_from(&lt, timestamp / 1000);
    atlantis_format_human_time(&lt, buf, len);
}
...
static void rebuild_history_array(void)
{
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        if (!ATLANTIS_BACKEND(app)->file_exists(e->key))
            continue;

        HistoryEntry* entry = &app.history[app.hist_count];
        entry->path = atlantis_strdup(e->key);
        entry->title = e->value ? atlantis_strdup(e->value) : NULL;

        char date_buf[64];
        format_date(e->timestamp, date_buf, sizeof(date_buf));
        entry->date_str = atlantis_strdup(date_buf);
...
{
    size_t len;
    char* content = ATLANTIS_BACKEND(app)->read_file(sessions_file_path(), &len);
    if (!content)
        return NULL;
...
static CrdtState* migrate_v1_to_crdt(const char* json, size_t len)
{
    ...
    cJSON_ArrayForEach(item, root)
    {
        ...
        if (!ATLANTIS_BACKEND(app)->file_exists(norm_path)) {
            free(norm_path);
            continue;
        }
...
void hist_load(void)
{
    ...
    if (ATLANTIS_BACKEND(app)->file_exists(legacy_history_path())) {
        size_t len;
        char* content = ATLANTIS_BACKEND(app)->read_file(legacy_history_path(), &len);
        if (content) {
            ...
            if (migrated) {
                ...
                if (json) {
                    ...
                    ATLANTIS_BACKEND(app)->write_file(sessions_file_path(), json, strlen(json));
                    free(json);
                }
                crdt_free(migrated);
            }
