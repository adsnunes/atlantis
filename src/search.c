...
char* search_web(const char* query)
{
    ...
    if (!curl)
        return atlantis_strdup("Search failed: could not initialize");
...
char* search_tool_callback(const char* params_json, void* user_data)
{
    ...
    if (!params)
        return atlantis_strdup("{\"error\": \"Invalid parameters\"}");

    cJSON* query = cJSON_GetObjectItem(params, "query");
    if (!query || !query->valuestring) {
        cJSON_Delete(params);
        return atlantis_strdup("{\"error\": \"Missing query parameter\"}");
    }
...
char* sessions_tool_callback(const char* params_json, void* user_data)
{
    ...
    if (!history_dir) {
        return atlantis_strdup("{\"error\": \"History directory not configured\"}");
    }
