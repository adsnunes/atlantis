...
#ifdef _WIN32
#define ATLANTIS_PATH_SEP "\\"
#else
#define ATLANTIS_PATH_SEP "/"
#endif

//! Build a legacy ~/.atlantis path (the pre-XDG default).
static void build_legacy_dir(char* out, size_t out_size)
{
    const char* home = ATLANTIS_BACKEND(app)->home_dir();
    ATLANTIS_ASSERT(home, "home_dir() returned NULL");
    snprintf(out, out_size, "%s" ATLANTIS_PATH_SEP "%s", home, HISTORY_DIR_NAME);
}
...
char* history_dir(void)
{
    ...
    if (ATLANTIS_BACKEND(app)->file_exists(legacy)) {
...
{
    return ATLANTIS_BACKEND(app)->username();
}
...
void save_session(void)
{
    ...
    ATLANTIS_BACKEND(app)->write_file(app.session_path, content, pos + txt_len);
    ...
    if (app.chat_count > 0) {
        ...
        if (json) {
            ATLANTIS_BACKEND(app)->write_file(chat_path, json, strlen(json));
            free(json);
        }
    }
}
...
void load_chat_history(const char* session_path)
{
    ...
    size_t size;
    char* json_str = ATLANTIS_BACKEND(app)->read_file(chat_path, &size);
    if (!json_str)
        return;
...
static void load_content(char* content, size_t size, const char* path)
{
    ...
    if (text_len > 0) {
        ...
        char* mutable_text = atlantis_strdup(text_start);
    ...
    app.session_path = path ? atlantis_strdup(path) : NULL;
    app.cursor = 0;
    app.scroll_y = 0;
    app.selecting = false;
    ...
    ATLANTIS_BACKEND(app)->set_title(title);
}

void load_file_for_editing(const char* path)
{
    size_t size;
    char* content = ATLANTIS_BACKEND(app)->read_file(path, &size);
    if (!content)
        return;
...
{
    ATLANTIS_BACKEND(app)->reveal(path);
}
