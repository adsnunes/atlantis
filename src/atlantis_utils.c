...
void atlantis_strcpy(char* dest, const char* src)
{
    size_t len = strlen(src);
    memcpy(dest, src, len + 1);
}

char* atlantis_strdup(const char* string)
{
    size_t length = 0;
    char* copy = NULL;
...
void atlantis_strncpy(char* dest, const char* src, size_t n)
{
    size_t len = strlen(src);
    if (len > n)
        len = n;
...
int32_t output_grapheme(const GapBuffer* gb, size_t* pos, MdStyle active_style)
{
    ...
    bool needs_scaling = (current_text_scale > 1 || (current_frac_num > 0 && current_frac_denom > 0))
        && atlantis_ctx_has(&app.ctx, ATLANTIS_CAP_TEXT_SIZING);
    ...
    if (replacement) {
        if (needs_scaling) {
            ...
            *pos += consumed;
            ...
            ATLANTIS_BACKEND(app)->write_str(replacement, strlen(replacement));
            *pos += consumed;
            return 1;
        }
    }
    ...
    if ((first & 0x80) == 0) {
        if (needs_scaling) {
            ...
            ATLANTIS_BACKEND(app)->write_char(first);
            (*pos)++;
            return 1;
        }
    }
    ...
    if (needs_scaling) {
        ...
        *pos += (size_t)n;
        ...
        ATLANTIS_BACKEND(app)->write_str((const char*)bytes, (size_t)n);
        *pos += (size_t)n;
        return width;
    }
}
...
int32_t output_grapheme_str(const char* text, size_t len, size_t* pos)
{
    ...
    // ASCII fast path
    if ((first & 0x80) == 0) {
        ATLANTIS_BACKEND(app)->write_char((char)first);
        (*pos)++;
        return 1;
    }
    ...
    // Output
    ATLANTIS_BACKEND(app)->write_str(&text[*pos], (size_t)n);
    *pos += (size_t)n;
