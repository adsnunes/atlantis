// atlantis_crdt.c - Generic LWW-Element-Map CRDT

#include "atlantis_crdt.h"
#include "cJSON.h"
...
{
    uint64_t r = 0;
    int64_t ms = ATLANTIS_BACKEND(app)->clock(ATLANTIS_CLOCK_MS);
    r ^= (uint64_t)ms;
...
{
    return ATLANTIS_BACKEND(app)->clock(ATLANTIS_CLOCK_SEC) * 1000;
}
...
CrdtState* crdt_parse(const char* json, size_t len)
{
    ...
    if (node && cJSON_IsString(node)) {
        atlantis_strncpy(state->node, node->valuestring, CRDT_NODE_ID_LEN);
    }
    ...
    if (entries && cJSON_IsObject(entries)) {
        ...
        if (count > 0) {
            ...
            cJSON_ArrayForEach(entry, entries)
            {
                ...
                e->key = atlantis_strdup(key);
                e->value = (value_j && cJSON_IsString(value_j)) ? atlantis_strdup(value_j->valuestring) : NULL;
                ...
                if (node_j && cJSON_IsString(node_j)) {
                    atlantis_strncpy(e->node, node_j->valuestring, CRDT_NODE_ID_LEN);
                }
                state->entry_count++;
            }
        }
    ...
    if (tombstones && cJSON_IsObject(tombstones)) {
        ...
        if (count > 0) {
            ...
            cJSON_ArrayForEach(tomb, tombstones)
            {
                ...
                t->key = atlantis_strdup(key);
                t->timestamp = (ts_j && cJSON_IsNumber(ts_j)) ? (int64_t)ts_j->valuedouble : 0;
                if (node_j && cJSON_IsString(node_j)) {
                    atlantis_strncpy(t->node, node_j->valuestring, CRDT_NODE_ID_LEN);
                }
...
CrdtState* crdt_merge(const CrdtState* a, const CrdtState* b)
{
    ...
    atlantis_strncpy(result->node, a->node, CRDT_NODE_ID_LEN);
    ...
    for (int32_t i = 0; i < key_count; i++) {
        ...
        if (best_entry && best_tomb) {
            ...
            if (cmp >= 0) {
                ...
                e->key = atlantis_strdup(best_entry->key);
                e->value = best_entry->value ? atlantis_strdup(best_entry->value) : NULL;
                e->meta = best_entry->meta ? cJSON_Duplicate(best_entry->meta, 1) : NULL;
                e->timestamp = best_entry->timestamp;
                atlantis_strncpy(e->node, best_entry->node, CRDT_NODE_ID_LEN);
            } else {
                CrdtTombstone* t = &result->tombstones[result->tombstone_count++];
                t->key = atlantis_strdup(best_tomb->key);
                t->timestamp = best_tomb->timestamp;
                atlantis_strncpy(t->node, best_tomb->node, CRDT_NODE_ID_LEN);
            }
        } else if (best_entry) {
            CrdtEntry* e = &result->entries[result->entry_count++];
            e->key = atlantis_strdup(best_entry->key);
...
void crdt_upsert(CrdtState* state, const char* key, const char* value)
{
    ...
    if (existing) {
        free(existing->value);
        existing->value = value ? atlantis_strdup(value) : NULL;
        existing->timestamp = ts;
        atlantis_strncpy(existing->node, state->node, CRDT_NODE_ID_LEN);
    } else {
        state->entries = realloc(state->entries, sizeof(CrdtEntry) * (size_t)(state->entry_count + 1));
        CrdtEntry* e = &state->entries[state->entry_count++];
        e->key = atlantis_strdup(key);
        e->value = value ? atlantis_strdup(value) : NULL;
        e->meta = NULL;
        e->timestamp = ts;
        atlantis_strncpy(e->node, state->node, CRDT_NODE_ID_LEN);
    }
}
...
void crdt_remove(CrdtState* state, const char* key)
{
    ...
    if (existing) {
        ...
        t->key = atlantis_strdup(key);
        t->timestamp = ts;
        atlantis_strncpy(t->node, state->node, CRDT_NODE_ID_LEN);
    }
}
