...
static char* settings_path(void)
{
    ...
    snprintf(path, sizeof(path), "%s" ATLANTIS_SEP "settings.json", config_dir());
    return path;
}
...
{
    size_t len;
    char* json = ATLANTIS_BACKEND(app)->read_file(settings_path(), &len);
    if (!json)
        return;
...
void settings_save(void)
{
    ...
    ATLANTIS_BACKEND(app)->mkdir_p(config_dir());
    ATLANTIS_BACKEND(app)->write_file(settings_path(), json, strlen(json));
    free(json);
}
