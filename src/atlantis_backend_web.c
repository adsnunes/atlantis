// atlantis_backend_web.c - Web/Emscripten Platform Backend
...
#include "atlantis_wrap.h" // for utf8_display_width
...
// When building for web, we compile atlantis_backend_web.c (not atlantis_backend_posix.c),
...
EM_JS(void, js_init_canvas, (), {
    let canvas = document.getElementById('atlantis-canvas');
    if (!canvas) {
        canvas = document.createElement('canvas');
        canvas.id = 'atlantis-canvas';
    ...
    document.body.style.backgroundAtlantisColor = '#1a1a2e';

    canvas.style.display = 'block';

    window.atlantisDPR = window.devicePixelRatio || 1;
    window.atlantisCtx = canvas.getContext('2d');
    window.atlantisFontSize = 14;

    const updateSize = () =>
    {
        const dpr = window.atlantisDPR;
        ...
        window.atlantisCtx.setTransform(dpr, 0, 0, dpr, 0, 0);

        const font = window.atlantisFontSize + 'px "SF Mono", "Monaco", "Menlo", "Consolas", "DejaVu Sans Mono", monospace';
        window.atlantisCtx.font = font;
        window.atlantisCtx.textBaseline = 'top';

        const metrics = window.atlantisCtx.measureText('M');
        window.atlantisCellWidth = Math.ceil(metrics.width);
        window.atlantisCellHeight = window.atlantisFontSize + 4;

        window.atlantisCols = Math.floor(width / window.atlantisCellWidth);
        window.atlantisRows = Math.floor(height / window.atlantisCellHeight);

        if (Module._web_on_resize) {
            Module._web_on_resize(window.atlantisCols, window.atlantisRows);
       }
   };
...
EM_JS(void, js_get_size, (int32_t* cols, int32_t* rows), {
    setValue(cols, window.atlantisCols || 80, 'i32');
    setValue(rows, window.atlantisRows || 24, 'i32');
});

EM_JS(void, js_clear_screen, (int32_t r, int32_t g, int32_t b), {
    const ctx = window.atlantisCtx;
...
EM_JS(void, js_set_font, (int32_t bold, int32_t italic), {
    ...
    window.atlantisCtx.font = style + window.atlantisFontSize + "px 'SF Mono', 'Monaco', 'Menlo', 'Consolas', 'DejaVu Sans Mono', monospace";
});
...
EM_JS(void, js_draw_text_scaled, (int32_t col, int32_t row, const char* text, int32_t scale, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim), {
    ...
    const x = (col - 1) * window.atlantisCellWidth;
    const y = (row - 1) * window.atlantisCellHeight;

    const scaledFontSize = window.atlantisFontSize * scale;
    const width = str.length * scale * window.atlantisCellWidth;
    const height = scale * window.atlantisCellHeight;
    ...
    ctx.font = window.atlantisFontSize + "px 'SF Mono', 'Monaco', 'Menlo', 'Consolas', 'DejaVu Sans Mono', monospace";
});
...
EM_JS(void, js_draw_text_scaled_frac, (int32_t col, int32_t row, const char* text, int32_t cell_scale, double font_scale, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim), {
    ...
    const scaledFontSize = window.atlantisFontSize * font_scale;
    const width = str.length * cell_scale * window.atlantisCellWidth;
    const height = cell_scale * window.atlantisCellHeight;
...
EM_JS(void, js_draw_text, (int32_t col, int32_t row, const char* text, int32_t num_cols, int32_t fg_r, int32_t fg_g, int32_t fg_b, int32_t bg_r, int32_t bg_g, int32_t bg_b, int32_t bold, int32_t italic, int32_t dim, int32_t underline, int32_t strikethrough), {
    ...
    const x = (col - 1) * window.atlantisCellWidth;
    const y = (row - 1) * window.atlantisCellHeight;
    const width = num_cols * window.atlantisCellWidth;

    if (bg_r >= 0) {
        ctx.fillStyle = `rgb(${bg_r}, ${bg_g}, ${bg_b})`;
        ctx.fillRect(x, y, width, window.atlantisCellHeight);
   }

    const style = (italic ? "italic " : "") + (bold ? "bold " : "");
    ctx.font = style + window.atlantisFontSize + "px 'SF Mono', 'Monaco', 'Menlo', 'Consolas', 'DejaVu Sans Mono', monospace";
    ...
    if (underline) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(x, y + window.atlantisCellHeight - 2);
        ctx.lineTo(x + width, y + window.atlantisCellHeight - 2);
        ctx.stroke();
   }
...
EM_JS(void, js_draw_cursor, (int32_t col, int32_t row, int32_t r, int32_t g, int32_t b), {
    ...
    if (window.atlantisDebug) {
        ctx.fillStyle = 'rgba(0,0,0,0.8)';
        ctx.fillRect(5, 5, 120, 40);
        ctx.fillStyle = 'rgba(0,255,0,0.9)';
        ctx.font = '12px monospace';
        ctx.fillText('R' + row + ' C' + col, 10, 22);
        ctx.fillText('Rows:' + window.atlantisRows, 10, 38);
   }

    ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
    ctx.fillRect(x, y, 2, window.atlantisCellHeight);
});
...
EM_JS(void, js_clear_rect, (int32_t col, int32_t row, int32_t width, int32_t height, int32_t r, int32_t g, int32_t b), {
    ...
    ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
    ctx.fillRect(x, y, width * window.atlantisCellWidth, height * window.atlantisCellHeight);
});

EM_JS(void, js_setup_input, (), {
    const canvas = document.getElementById('atlantis-canvas');
    ...
    canvas.addEventListener('mousedown', (e) => {
        const col = Math.floor(e.offsetX / window.atlantisCellWidth) + 1;
        const row = Math.floor(e.offsetY / window.atlantisCellHeight) + 1;
...
EM_JS(void, js_init_filesystem, (), {
    // Create /atlantis directory if it doesn't exist
    try
    {
        FS.mkdir('/atlantis');
   }
...
EM_JS(int32_t, js_display_image, (const char* path, int32_t row, int32_t col, int32_t maxWidth, int32_t maxHeight), {
    ...
    let img = window.atlantisImages ?.[pathStr];
    if (!img) {
        window.atlantisImages = window.atlantisImages || {};
        img = new Image();
        img.src = pathStr;
        window.atlantisImages[pathStr] = img;
    ...
    const maxW = maxWidth * window.atlantisCellWidth;
    const maxH = maxHeight > 0 ? maxHeight * window.atlantisCellHeight : img.naturalHeight;
    ...
    return Math.ceil(h / window.atlantisCellHeight);
});
...
EM_JS(const char*, js_get_username, (), {
    ...
    const name = localStorage.getItem('atlantis_username') || 'Writer';
...
void web_on_mouse(int32_t col, int32_t row, int32_t button, int32_t pressed)
{
    ...
    if (pressed && button == 0) {
        int32_t next_tail = (web_state.key_queue_tail + 1) % 64;
        if (next_tail != web_state.key_queue_head) {
            web_state.key_queue[web_state.key_queue_tail] = ATLANTIS_KEY_MOUSE_CLICK;
            web_state.key_queue_tail = next_tail;
       }
   }
}

static bool web_init(AtlantisMode mode)
{
    ...
    web_state.capabilities = ATLANTIS_CAP_TRUE_COLOR | ATLANTIS_CAP_SYNC_OUTPUT | ATLANTIS_CAP_STYLED_UNDERLINE | ATLANTIS_CAP_TEXT_SIZING | ATLANTIS_CAP_IMAGES | ATLANTIS_CAP_MOUSE | ATLANTIS_CAP_BRACKETED_PASTE | ATLANTIS_CAP_CLIPBOARD;
...
static void web_set_fg(AtlantisColor color)
{
...
static void web_set_bg(AtlantisColor color)
{
...
static void web_set_underline(AtlantisUnderline style)
{
    web_state.underline = true;
    web_state.underline_style = style;
}

static void web_set_underline_color(AtlantisColor color)
{
    ...
    *out_count = 0;
    ...
    int32_t count = EM_ASM_INT({
        try {
            ...
            window.atlantisDirEntries = entries;
            return entries.length;
}
catch(e)
{
    window.atlantisDirEntries = [];
    return 0;
}
}, path);
...
for (int32_t i = 0; i < count; i++) {
    (*out_names)[i] = (char*)EM_ASM_PTR({
            const name = window.atlantisDirEntries[$0];
            const len = lengthBytesUTF8(name) + 1;
            const ptr = _malloc(len);
            stringToUTF8(name, ptr, len);
            return ptr;}, i);
...
static int64_t web_clock(AtlantisClock kind)
{
    if (kind == ATLANTIS_CLOCK_MS) {
...
static void web_get_local_time(AtlantisTime* out)
{
...
static void web_get_local_time_from(AtlantisTime* out, int64_t timestamp)
{
    if (!out)
        return;
...
{
    EM_ASM({
        window.atlantisImages = {};
   });
}

static void web_image_mask_region(int32_t col, int32_t row, int32_t cols, int32_t rows, AtlantisColor bg)
{
    ...
    char* out, size_t out_size)
{
    ...
    if (base_dir) {
        ...
        snprintf(out, out_size, "/atlantis/%s", raw_path);
   }
...
static void web_image_invalidate(const char* path)
{
    EM_ASM({
        const path = UTF8ToString($0);
        if (window.atlantisImages) {
            delete window.atlantisImages[path];
       }}, path);
}
...
const AtlantisBackend atlantis_backend_web = {
    .name = "web",
