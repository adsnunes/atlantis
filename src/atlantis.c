// atlantis.c
//! Frontends call into this via atlantis_app.h API

#include "atlantis_app.h"
#include "atlantis_history.h"
#include "atlantis_types.h"

#include "atlantis_block.h"
#include "atlantis_chat.h"
#include "atlantis_clipboard.h"
#include "atlantis_date.h"
#include "atlantis_file.h"
#include "atlantis_fm.h"
#include "atlantis_footnote.h"
#include "atlantis_gap.h"
#include "atlantis_highlight.h"
#include "atlantis_image.h"
#include "atlantis_input.h"
#include "atlantis_nav.h"
#include "atlantis_render.h"
#include "atlantis_search.h"
#include "atlantis_settings.h"
#include "atlantis_tex.h"
...
#define HAS_CAP(cap) atlantis_ctx_has(&app.ctx, cap)
...
#define IS_PRINTABLE(k) ((k) >= 32 && (k) != 127 && (k) < ATLANTIS_KEY_UP)
...
#define IS_PRINT_MODE() (app.ctx.mode == ATLANTIS_MODE_PRINT)
...
#define GET_LINE_SCALE(line_style) \
    (HAS_CAP(ATLANTIS_CAP_TEXT_SIZING) ? block_get_scale(line_style) : 1)
...
static void update_title(void)
{
    switch (app.mode) {
    case MODE_WELCOME:
        ATLANTIS_BACKEND(app)->set_title("Atlantis");
        break;
    case MODE_HISTORY:
        ATLANTIS_BACKEND(app)->set_title("Atlantis | History");
        break;
    case MODE_TIMER_SELECT:
        ATLANTIS_BACKEND(app)->set_title("Atlantis | Timer");
        break;
    case MODE_HELP:
        ATLANTIS_BACKEND(app)->set_title("Atlantis | Help");
        break;
    case MODE_WRITING:
    case MODE_FM_EDIT:
    ...
    case MODE_TOC:
    case MODE_SEARCH: {
        // Use document title if available, otherwise "Atlantis"
        const char* title = fm_get_string(app.frontmatter, "title");
        ATLANTIS_BACKEND(app)->set_title(title ? title : "Atlantis");
        break;
    }
    default:
        ATLANTIS_BACKEND(app)->set_title("Atlantis");
...
{
    if (!atlantis_parse_iso_date(s, &dt->d))
        return false;
    dt->part = 0;
    return true;
}
...
static void fm_edit_save(void)
{
    ...
    for (int32_t i = 0; i < app.fm_edit.field_count; i++) {
        ...
        case FM_FIELD_DATETIME: {
            char buf[64];
            atlantis_format_iso_date(&field->datetime.d, buf, sizeof(buf));
    ...
    AtlantisTime lt;
    ATLANTIS_BACKEND(app)->localtime(&lt);
    char lastmod_buf[32];
    atlantis_format_iso_time(&lt, lastmod_buf, sizeof(lastmod_buf));
...
static bool render_image_element(const RenderCtx* ctx, RenderState* rs, const Block* block)
{
    ...
    if (!HAS_CAP(ATLANTIS_CAP_IMAGES)) {
...
static void render_status_bar(const Layout* L)
{
    ...
    if (app.timer_mins > 0 && app.timer_on) {
        ...
        AtlantisColor tc = color_lerp(get_dim(), get_accent(), prog);
        set_fg(tc);
        if (app.timer_paused)
            out_str("⏸ ");
        char time_buf[16];
...
static bool render_link(const RenderCtx* ctx, RenderState* rs, const InlineRun* run)
{
    ...
    while (link_pos < link_end) {
        ...
        if (IS_ROW_VISIBLE(&ctx->L, screen_row, ctx->max_row)) {
            ...
            if (!link_started) {
                ATLANTIS_BACKEND(app)->link_begin(url);
    ...
    size_t* seg_end, int32_t* seg_width)
{
    ...
    case BLOCK_HEADER: {
        if (!is_first_line) {
            ...
            int32_t header_scale = HAS_CAP(ATLANTIS_CAP_TEXT_SIZING) ? text_scale : 1;
...
static void render(void)
{
    ...
    case MODE_TOC:
        ...
        break;
    case MODE_SEARCH:
        search_find(&app.text, (SearchState*)app.search_state, ATLANTIS_BACKEND(app)->clock(ATLANTIS_CLOCK_MS));
        render_writing();
        render_search();
        break;
    }
...
static void new_session(void)
{
    ...
    // Generate path in .atlantis directory
    free(app.session_path);
    ATLANTIS_BACKEND(app)->mkdir_p(history_dir());
    AtlantisTime lt;
    ATLANTIS_BACKEND(app)->localtime(&lt);
    char timestamp[20];
    atlantis_format_filename_time(&lt, timestamp, sizeof(timestamp));
    char path[PATH_MAX];
#ifdef _WIN32
    ...
    snprintf(path, sizeof(path), "%s/%s.md", history_dir(), timestamp);
#endif
    app.session_path = atlantis_strdup(path);

    fm_free(app.frontmatter);
    app.frontmatter = fm_create();
    fm_set_string(app.frontmatter, "title", "Untitled");
    fm_set_string(app.frontmatter, "author", ATLANTIS_BACKEND(app)->username());
    char date_buf[32];
    atlantis_format_iso_time(&lt, date_buf, sizeof(date_buf));
    ...
    if (app.timer_on) {
        app.timer_start = ATLANTIS_BACKEND(app)->clock(ATLANTIS_CLOCK_SEC);
    }
...
static void handle_writing(int32_t key)
{
    ...
    case ATLANTIS_KEY_MOUSE_SCROLL_UP: {
        app.scroll_y -= 3;
        if (app.scroll_y < 0)
            app.scroll_y = 0;
        break;
    }
    case ATLANTIS_KEY_MOUSE_SCROLL_DOWN: {
        app.scroll_y += 3;
    ...
    default:
        if (!CAN_MODIFY())
            break;
        // Accept printable chars but not special keys (ATLANTIS_KEY_UP=1000 and above)
        if (IS_PRINTABLE(key)) {
            save_undo_state();
            delete_selection_if_any();
            uint8_t utf8_buf[4];
...
static void handle_ai_input(int32_t key)
{
    ...
    case ATLANTIS_KEY_PGUP:
    case ATLANTIS_KEY_MOUSE_SCROLL_UP:
        app.chat_scroll += (key == ATLANTIS_KEY_PGUP ? 10 : 3);
        break;
    case ATLANTIS_KEY_PGDN:
    case ATLANTIS_KEY_MOUSE_SCROLL_DOWN:
        app.chat_scroll -= (key == ATLANTIS_KEY_PGDN ? 10 : 3);
        if (app.chat_scroll < 0)
            app.chat_scroll = 0;
        break;
...
static void handle_input(void)
{
    ...
    case MODE_HISTORY:
        ...
        case 'd':
            if (app.hist_count > 0) {
                char* path = atlantis_strdup(app.history[app.hist_sel].path);
                hist_remove(path);
                free(path);
    ...
    case MODE_FM_EDIT: {
        ...
        if (field && field->kind == FM_FIELD_STRING) {
            ...
            if (key == '\r' || key == '\n') {
            ...
            } else if (key == ATLANTIS_KEY_UP || key == ATLANTIS_KEY_DOWN) {
                ...
                int32_t target_line = key == ATLANTIS_KEY_UP ? cur_line - 1 : cur_line + 1;
    ...
    case MODE_SEARCH: {
        ...
        case '\b':
            if (search->query_len > 0) {
                ...
                search_mark_dirty(search, ATLANTIS_BACKEND(app)->clock(ATLANTIS_CLOCK_MS));
            }
            break;
        default:
            if (IS_PRINTABLE(key)) {
...
{
    ATLANTIS_BACKEND(app)->set_title(NULL);
...
bool atlantis_frame(void)
{
    if (app.quit)
        return false;
    if (ATLANTIS_BACKEND(app)->check_quit())
        return false;
    if (ATLANTIS_BACKEND(app)->check_resize())
        atlantis_update_size();
    if (app.timer_on)
        timer_check();
...
void atlantis_request_quit(void) { app.quit = true; }
bool atlantis_should_quit(void) { return app.quit; }
...
void atlantis_new_document(void) { new_session(); }

void atlantis_save_document(void) { save_session(); }

void atlantis_update_size(void)
{
    ATLANTIS_BACKEND(app)->get_size(&app.cols, &app.rows);
}

void atlantis_render(void) { render(); }
...
static void render_writing(void)
{
    ...
    if (!print_mode && app.cursor >= len) {
        ...
        bool skip = last && app.cursor == last->end && !has_newline && (last->type == BLOCK_HR || (last->type == BLOCK_HEADER && HAS_CAP(ATLANTIS_CAP_TEXT_SIZING)));
        if (!skip) {
            rs.cursor_virtual_row = running_vrow;
...
static void render_block(const RenderCtx* ctx, RenderState* rs, const Block* block)
{
    ...
    case BLOCK_HEADER:
        if (HAS_CAP(ATLANTIS_CAP_TEXT_SIZING)) {
    ...
    render_as_paragraph: {
        ...
        while (rs->pos < block->end && rs->pos < len) {
            ...
            if (rs->col_width == 0 && at_line_start) {
                ...
                if (!HAS_CAP(ATLANTIS_CAP_TEXT_SIZING) && block->type == BLOCK_HEADER) {
