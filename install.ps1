#!/usr/bin/env pwsh
# Atlantis installer for Windows
# Usage: irm https://raw.githubusercontent.com/adsnunes/atlantis/main/install.ps1 | iex

$ErrorActionPreference = 'Stop'
$ProgressPreference = 'SilentlyContinue'

$repo = "adsnunes/atlantis"
$installDir = "$env:USERPROFILE\.atlantis\bin"
...
$asset = $release.assets | Where-Object { $_.name -eq "atlantis-windows-$arch.zip" }
if (-not $asset) { Write-Err "No Windows $arch build available for $version" }

# Download
Write-Status "`u{1F4E6}" "Downloading atlantis $version for Windows $arch..."
$zipPath = "$env:TEMP\atlantis-windows-$arch.zip"
try {
...
if (-not (Test-Path "$installDir\atlantis.exe")) {
    Write-Err "Installation failed: atlantis.exe not found"
}
...
# Done
Write-Host ""
Write-Status "`u{2705}" "atlantis $version installed successfully!"
Write-Host ""
Write-Host "  Run " -NoNewline
Write-Host "atlantis" -ForegroundColor Cyan -NoNewline
